...
public partial class Program
{
  public static int Main(string[] args)
  {
    ...
    using (StreamReader sr = File.OpenText(filePath))
    {
      ...
      while (null != (vBuf = sr.ReadLine()))
      {
        try
        {
          ...
          if ( 0 == matrixFloyd[fIndex, sIndex])
          {
            ...
            smezhn1.Add(new Edge(fIndex, sIndex, value));
          }
          else
          {
            ...
            for (int i = 0; i < smezhn1.Count; i++)
            {
              if (smezhn1[i].from == fIndex && smezhn1[i].to == sIndex
               || smezhn1[i].from == sIndex && smezhn1[i].to == fIndex)
              {
                smezhn1[i] = new Edge(fIndex, sIndex, matrixFloyd[fIndex, sIndex]);
              }
            }
          }
        }
      ...
      smezhn = smezhn1.ToArray();

      sr.Close();
    }

    Floyd(matrixFloyd, vNum);

    int resultPrim = Prim(matrixPrim, vNum);
    int resultKruskal = Kruskal(smezhn, vNum);
